*************************************	Homework 1	************************************************************
Aim: Given a dictionary of 10000 words, implement the SHARK2 algorithm to decode a user input gesture and
     output the best decoded word.

Implementation: The complete implementation of this homework is with the help of the SHARK2 publication authored
		by Dr. Per Ola Kristensson and Dr. Shumin Zhai.

Steps:

1. Sampling: SHARK2 performs comparations between the user input pattern and standard templates of word. But
...
2. Pruning: Pruning refers to the process of computing the start-to-start and end-to-end distances between a 
...
3. Shape Score: Before we generate the shape score, we need to normalize both the points of the valid words 
...
4. Location Score: This step is the most time-consuming step, as we do 100*100*number_of_templates calculation
...
5. Integration Score: Final step is to compute the integration score, which is given by the formula:
...
Output: The final output is the word with the lowest integration score. If there is a tie, we return all words,
